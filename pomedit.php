...
$title = $pomRow['title'];
$data = $pomRow['data'];
$miasto = $pomRow['miejscowosc'];
$ilosc = $pomRow['stawka'];
$dod = $pomRow['dodatkowe'];
$desc = $pomRow['opis'];
$za = $pomRow['za'];
...
$query = "UPDATE `pomoc` SET `title` = '$title',`dodane` = '$dodane',`data` = '$date',`miejscowosc` = '$place',`stawka` = '$ilosc',`za` = '$za',`dodatkowe` = '$dod', `opis` = '$desc' WHERE `rand`='$ran'";
$res = mysql_query($query);
   ...
   if ($res) {
	...
	unset($miasto);
	unset($data);
    $errMSG = "Dziękujemy! <a href=".$linkacz.$rand.">Ogłoszenie</a> zostało zedytowane!"; 
...
$('#charNum').css('color', 'black');
            ...
            <input type="text" name="miasto" id="miasto" value="<?php echo $miasto; ?>" class="form-control" placeholder="Miejscowość" >
                </div>
...
<script>
  var optionToSet = <?php echo json_encode($pomRow['za']); ?>;
  var a = parseInt(optionToSet);
$('#czas').val(a);
...
<textarea style="width: 85%;" class="form-control"  rows="6" name="desc" id="desc" placeholder="Opis" maxlength="500" data-toggle="tooltip" data-placement="right" title="Uwaga! Chrome ma problem z rozpoznawaniem enterów. Przy użyciu entera liczba pozostałych znaków może się różnić od prawdziwej! "><?php echo $desc; ?></textarea>
...
$('#descNum').css('color', 'black');
		   ...
		   <div class="form-group" style="margin-top: 10px;">
			 ...
			 <div class="btn-group btn-group-justified" style="margin-top: 5px; width:70%;">
			  <a href="pomhome" class="btn btn-danger">Wszystkie ogłoszenia</a>
  <a href="pomoffer?ran=<?php echo $ran; ?>" class="btn btn-danger">Przejdź do ogłoszenia</a>
 </div>
