<?php
 ...
 if( !isset($_SESSION['user']) ) {
  header("Location: login?continue=pommine");
  exit;
 }
...
$linek = 'usprofile?ran='.$userRow['profilehash'];
 	...
 	if( isset($_POST['edit']) ) { 
	$value = $_POST['edit'];
	$_SESSION["pomrand"] = $value;
	header("Location: pomedit");
	}
	...
	$sq = "SET @count = 0";
	mysql_query($sq);
	$s = "UPDATE `pomoc` SET `pomoc`.`ID` = @count:= @count + 1";
	mysql_query($s);
	if($s){
	...
	$random = $_POST['wyr'];
	$wyrozniam = ("UPDATE pomoc SET wyr=1 WHERE rand='$random'");
...
if($wyres){
	...
	$random = $_POST['uwyr'];
	$wyrozniam = ("UPDATE pomoc SET wyr=0 WHERE rand='$random'");
	$wyres = mysql_query($wyrozniam);
if($wyres){
...
$cz = "SELECT wyr from pomoc where kto='$nazwa' and wyr=1";
$cze = mysql_query($cz);
$dzisia = date("Y-m-d");
$wygas = $premRow['do'];
      ...
      <ul class="nav navbar-nav navbar-right">
        <li class="dropdown">
		  ...
		  <li><a href="<?php echo $linek; ?>">Mój profil</a></li>
            <li class="active"><a href="pommine">Moje ogłoszenia</a></li>
...
<div class="col-md-10 col-md-offset-1"><input style="margin-bottom:30px;" id="mine" name="search" type="text" placeholder="słowo kluczowe,firma,miasto" autocomplete="off"><input id="submit" ></div>
 <br><div class="row">
 <div class="col-md-10 tabela col-md-offset-1"><div class="table-responsive">
 ...
 if (strpos($a, '1') !== false) {
    ...
    $za = "całość pracy";
            ...
            ?>
                <tr>
				<td><p style="float: left"><a href="<?php echo 'http://mrwork.pl/pomoffer?ran='.$row['rand']; ?>"><?php echo $row['title'] ?></a><br>Dodano: <?php echo $row['dodane'] ?><br><i class="fa fa-map-marker" aria-hidden="true"></i>&nbsp;<?php echo $row['miejscowosc'] ?></p>
				<p style="margin: 0;"><div style="float: right"><?php echo $row['stawka'] ?>zł za <?php echo $za ?></div><br>&nbsp;<br><div style="float: right">
...
$(".stopka").css("marginTop", ile);
 ...
 <div class="stopka row"><footer class="footer-distributed">
