<?php
 ...
 if( !isset($_SESSION['user']) ) {
  header("Location: login?continue=pomoffer?ran=$abc");
  exit;
 }
   $po = "SELECT rand from pomoc where rand='$abc'";
$pp = mysql_query($po);
...
$linek = 'usprofile?ran='.$userRow['profilehash'];
 ...
 $ran = substr($server, 4);
 $pom=mysql_query("SELECT * FROM pomoc WHERE rand='$ran'");
 $pomRow=mysql_fetch_array($pom);
 $next = $pomRow['ID'] + 1;
 $nex=mysql_query("SELECT rand FROM pomoc WHERE ID='$next'");
 $nextRow=mysql_fetch_array($nex); 
 $_SESSION["pomrand"] = $ran;
 $a = $pomRow['za'];
 $f = $pomRow['dodatkowe'];
if(empty($f)){
$er = true;
...
$u = $pomRow['kto'];
$ue=mysql_query("SELECT * FROM users WHERE userName='$u'");
 $ueRow=mysql_fetch_array($ue); 
    $uemine = $ueRow['pracodawca'];
if (strpos($uemine, '1') !== false) {
    $linka = 'profile?ran='.$ueRow['profilehash'];
}
else {
$linka = 'usprofile?ran='.$ueRow['profilehash'];
}
 $numer = $ueRow['pracodawca'];
 if (strpos($numer, '1') !== false) {
    $pra=mysql_query("SELECT * FROM pradet WHERE name='$u'");
...
$uz=mysql_query("SELECT * FROM userdet WHERE name='$u'");
...
<title><?php echo $pomRow['title']; ?></title>
...
<nav class="navbar naw navbar-default">
    ...
    echo "display:block;";
		...
		<li><a href="pomadd"><span class="glyphicon glyphicon-plus"></span> Dodaj ogłoszenie</a></li>
...
<div class="row">
    <div class="col-sm-12"><a href="#" onclick="window.history.back();" style="float: left; font-weight: 700; font-family: Raleway; font-size:15px;"><i class="fa fa-angle-left" aria-hidden="true"></i>&nbsp;Wróć</a>
	<?php if (mysql_num_rows($nex) != 0) { ?><div><a href="pomoffer?ran=<?php echo $nextRow['rand']; ?>" style="float: right; font-weight: 700; font-family: Raleway; font-size:15px;">Następne ogłoszenie&nbsp;<i class="fa fa-angle-right" aria-hidden="true"></i></a></div><?php } ?>
	</div>
	</div>
	<hr style="margin-bottom:10px; margin-top: 10px !important; border-top: 1px solid #D8D8D8 !important;">
    <div class="row"><div class="col-sm-8"><h2 style="margin-top:5px;word-break: keep-all;"><?php  echo $pomRow['title']; ?>&nbsp;<?php $var1 = $pomRow['kto']; $var2 = $userRow['userName']; if (strcmp($var1, $var2) == 0) { echo "<a href='pomedit' style='font-size:12px;'>Edytuj</a>"; } ?></h2>
	<h4><i class="fa fa-map-marker" aria-hidden="true"></i>&nbsp;<?php echo $pomRow['miejscowosc']; ?><br><span style="font-size:14px; font-weight: 200;">Dodano: <?php echo $pomRow['dodane']; ?>,ID ogłoszenia: <?php echo $pomRow['rand']; ?> </span></h4>
	<ul class="list-inline" style="margin-top:-5px;">
  <li><b>W dniu:</b>&nbsp;<?php echo $datka; ?></li>
 <br> <li><b>Stawka:</b>&nbsp;<?php echo $pomRow['stawka']; echo"zł"; echo " "; echo "za"; echo " ";  echo $za; ?></li></ul>
	</div>
    <div class="col-sm-4" style="margin-top:10px;"><a <?php if (strcmp($ten, $nazwa) != 0) { ?>data-toggle="modal" data-target="#mess"<?php } ?> <?php if (strcmp($ten, $nazwa) == 0) { echo "style='pointer-events: none; cursor: default;'";} else { echo "style='cursor: pointer;'";}?>>
			...
			<div class="osoba" style="margin-top:5px;">
				<p style="color:#fff; font-weight: 600; text-align: center; vertical-align: middle; margin-top: 10px;"><span class="glyphicon glyphicon-user" aria-hidden="true"></span>&nbsp;<?php echo $pomRow['kto']; ?></p> 
				</div></a>
				<div class="osoba" style="margin-top:5px;">
...
<div class="row">
    <div class="col-sm-8"><div class="opis"><p style="margin: 5px;"><?php echo $pomRow['opis']; ?></p><p style="<?php if($er) { echo "display:none;"; } ?>"><b>Dodatkowe wymagania:</b>&nbsp;<?php echo $pomRow['dodatkowe']; ?></p></div>
