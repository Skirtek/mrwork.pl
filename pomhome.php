<?php
 ...
 if( !isset($_SESSION['user']) ) {
  header("Location: login?continue=pomhome");
  exit;
 }
...
<title>Pomoc sąsiedzka - strona główna</title>
...
<?php
   ...
   ?>
<?php if($premium === "true") {?><div class="col-md-12"><input style="margin-bottom:50px;" id="mine" name="search" type="text" placeholder="słowo kluczowe,nazwa,miasto" autocomplete="off"><input id="submit" ></div>
 <br><?php } ?><div class="row"><div class="col-md-10 tabela col-md-offset-1"><div class="table-responsive">
  <table id="example" class="display table table-hover" cellspacing="0" width="100%">
        ...
        <tbody>
		 <?php
			mysql_query("SET NAMES utf8");
			$sql = "SELECT * FROM pomoc WHERE wyr=1 order by ID desc";
   ...
   $ppra = $linkaRow['pracodawca'];
   if($ppra === "1"){
   $linek = "profile?ran=".$linkaRow['profilehash'];
   }
   else {
   $linek = "usprofile?ran=".$linkaRow['profilehash'];
   }
...
header("Refresh:0");
 ...
 if (strpos($a, '1') !== false) {
    ...
    $za = "całość pracy";
            ...
            ?>
                <tr>
				<td style=" background-color: #f4f495; border-bottom: 1px solid #FBFAFA;"><p style="float: left"><span style=" color: #058547; font-family: Raleway; font-weight: 700; "><i class="fa fa-star" aria-hidden="true"></i>&nbsp;Ogłoszenie wyróżnione</span><br><a href="<?php echo 'http://mrwork.pl/pomoffer?ran='.$row['rand']; ?>"><?php echo $row['title'] ?></a><br><i class="fa fa-user" aria-hidden="true"></i>&nbsp;<a style="color:#000;" href="<?php echo $linek;?>"><?php echo $row['kto'] ?></a><br><i class="fa fa-map-marker" aria-hidden="true"></i>&nbsp;<?php echo $row['miejscowosc'] ?></p><p style="margin: 0;"><div style="float: right"><?php echo $row['stawka'] ?>zł za <?php echo $za ?></div><br>&nbsp;<br><br><div style="float: right">Dodano: <?php echo $row['dodane'] ?></div></p></td>
                </tr>
 ...
 <?php
			mysql_query("SET NAMES utf8");
			$sql = "SELECT * FROM pomoc WHERE wyr=0 order by ID desc";
            $results = mysql_query($sql);
...
var myCallback = function () { 
    ...
    "emptyTable":     "Brak dostępnych ogłoszeń",
    "paginate": {
        "first":      "Pierwsza",
        "previous":   "Poprzednia",
        "next":       "Następna",
